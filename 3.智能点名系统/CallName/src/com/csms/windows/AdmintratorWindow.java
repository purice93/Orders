...
public class AdmintratorWindow extends JFrame implements ActionListener {
	...
	private JButton outToDatabaseBtn = new JButton("入数据库");
	private JButton termStateBtn = new JButton("期末报表");
	...
	public void rigthInformation() {
		...
		toolBar.add(this.outToDatabaseBtn);
		toolBar.add(this.termStateBtn);
		...
		this.outToDatabaseBtn.addActionListener(this);
		this.termStateBtn.addActionListener(this);
	...
	class StudentList extends AbstractTableModel {
		public String[] titles = { "学号", "姓名", "图像地址", "被叫次数", "迟到次数" };
	...
	public void actionPerformed(ActionEvent e) {
		...
		// 7.入数据库
		if (e.getSource() == this.outToDatabaseBtn) {
			Student stu = new Student();
			StudentDao sd = new StudentDao();
			for (int i = 0; i < pList.size(); i++) {
				stu.setStudentId(pList.get(i).getStudentId());
				stu.setStudentName(pList.get(i).getStudentName());
				stu.setStudentPic(pList.get(i).getStudentPic());
				stu.setCalledNumber(pList.get(i).getCalledNumber());
				stu.setLateNumber(pList.get(i).getLateNumber());
				sd.insertStudent(stu);
			}
			PublicWindowSet.promptPopUp("入数据库成功!!!", "提示", frame);
		}

		// 8.学期报表
		if (e.getSource() == this.termStateBtn) {
			Student student = null;
