...
public class EmailServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if("admin".equals((String)currentUserType)) {
				...
				request.setAttribute("recordList", recordList);
				request.setAttribute("mainPage", "dormManager/email.jsp");
	...
	private void sendEmail(HttpServletRequest request,
		...
		try {
			con = dbUtil.getCon();
			recordDao.sendEmail(con, studentNumber);
			request.getRequestDispatcher("email?action=list").forward(request, response);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			try {
	...
	private void recordSave(HttpServletRequest request,
		...
		try {
			...
			if(student.getName() == null) {
				...
				request.setAttribute("error", "学生姓名为空");
