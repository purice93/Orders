...
public class AdmintratorWindow extends JFrame implements ActionListener {
	private static final int WINDOWWIDE = 1100;
	...
	private List<User> gradeUserList = new LinkedList<>();// 读取的有分数用户
	...
	private JButton showPicBtn = new JButton("显示截图");
	...
	public void tableInformation() {
		...
		this.showPicBtn.addActionListener(this);
	...
	public void actionPerformed(ActionEvent e) {
		...
		// 显示操作流程表
			...
			List<String> list = new ArrayList<>();// 1.所有操作類型
			...
			// 数据源
			List<List<String>> opList = getOperationData(userList, list);// 2.所有的操作数据

			userDao.createDateBase(list); // a.创建操作数据库表
			userDao.insertStep(opList, list);// b.出入操作数据
		...
		// 显示登录信息/有成绩
			...
			userDao.insertHaveScore(HaveScoreLists);
		...
		// 显示登录信息/无成绩
			...
			userDao.insertNoScore(noScoreLists);
		...
		// 显示截图
		else if (e.getSource() == this.showPicBtn) {
	...
	private List<List<String>> getNoScoreData(List<User> userList2)
		...
		while (iterator.hasNext()) {
			...
			// 4.通过查看是否有成绩，来确定是否添加用户
			String gradeStr = userDao.selectScoreByUsername(username);
			if (gradeStr != null) {
				continue;// 如果有成绩，取消添加这个用户
			}
