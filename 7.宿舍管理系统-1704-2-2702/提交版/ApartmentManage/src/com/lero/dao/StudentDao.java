...
public class StudentDao {
	...
	public List<Student> studentList(Connection con, Student s_student)
			throws Exception {
		...
		while (rs.next()) {
			...
			student.setDormBuildName(DormBuildDao.dormBuildName(con,
					dormBuildId));
	...
	public static Student getNameById(Connection con, String studentNumber,
			int dormBuildId) throws Exception {
	...
	public boolean haveNameByNumber(Connection con, String studentNumber)
			throws Exception {
	...
	public List<Student> studentListWithBuild(Connection con,
			Student s_student, int buildId) throws Exception {
		List<Student> studentList = new ArrayList<Student>();
		StringBuffer sb = new StringBuffer("select * from t_student t1");
		if (StringUtil.isNotEmpty(s_student.getName())) {
			sb.append(" and t1.name like '%" + s_student.getName() + "%'");
	...
	public int studentCount(Connection con, Student s_student) throws Exception {
		StringBuffer sb = new StringBuffer(
				"select count(*) as total from t_student t1");
		if (StringUtil.isNotEmpty(s_student.getName())) {
			sb.append(" and t1.name like '%" + s_student.getName() + "%'");
		} else if (StringUtil.isNotEmpty(s_student.getStuNumber())) {
			sb.append(" and t1.stuNum like '%" + s_student.getStuNumber()
	...
	public Student getStudentByNumber(Connection con, String studentNumber) throws SQLException {
		...
		if (rs.next()) {
			student.setName(rs.getString("name"));
			student.setDormBuildId(rs.getInt("dormBuildId"));
			student.setInstructorId(rs.getString("instructorId"));
		}
		return student;
	}
