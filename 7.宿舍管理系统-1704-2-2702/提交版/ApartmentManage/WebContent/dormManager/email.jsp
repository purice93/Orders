...
window.onload = function(){ 
	...
	function sendEmail(studentNumber) {
		if(confirm("您确定要删除这条记录吗？")) {
			window.location="email?action=delete&studentNumber="+studentNumber;
			alert(studentNumber);
		}
	}
</script>
			...
			<table class="table table-striped table-bordered table-hover datatable">
				...
				<c:forEach  varStatus="i" var="record" items="${recordList }">
					<tr>
						...
						<td>${record.detail }</td>
						<td>
							<button class="btn btn-mini btn-danger" type="button" onclick="javascript:window.location='email?action=delete&studentNumber=${record.studentNumber }'">发送邮件</button></td>
					</tr>
				</c:forEach>
				</tbody>
			</table>
